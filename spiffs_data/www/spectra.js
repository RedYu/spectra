    ...
    function init_logger() {
        ...
        function parseBatch(buffer) {
            ...
            for (let i = 0; i < view.getUint16(2, true); i++) {
                if (offset + 40 > view.byteLength) throw new Error("Truncated CAN event");
                ...
                if (length > 64 || bus > 1 || type > 4 || source > 2 ||
                    offset + 40 + length > view.byteLength) throw new Error("Invalid CAN event");
                events.push({
            ...
            if (offset !== view.byteLength) throw new Error("Unexpected trailing CAN data");
            return events;
        }
        ...
        function acceptEvent(event) {
            ...
            if (channel.history.length > 500) channel.history.shift();
            // Count actual RX and completed TX, not every TX lifecycle event.
            if (event.type === 0 || event.type === 2) {
                if (event.type === 0) channel.rx++; else channel.tx++;
    ...
    function init_analyzer() {
        ...
        function validate(e) {
            if (e.bus > 1 || e.type > 4 || e.source > 2 || e.data.length > 64 ||
                e.id > (e.flags & 1 ? 0x1fffffff : 0x7ff)) throw new Error("Invalid CAN event fields");
            return e;
        }
        function parseLive(buffer) {
            const v = new DataView(buffer), result = [];
            if (v.byteLength < 8 || v.getUint8(0) !== 1 || v.getUint8(1) !== 1 ||
                v.getUint32(4, true) !== v.byteLength - 8) throw new Error("Invalid WebSocket batch");
            let o = 8;
            for (let i = 0; i < v.getUint16(2, true); i++) {
                if (o + 40 > v.byteLength) throw new Error("Truncated event");
                const n = v.getUint8(o + 5);
                if (n > 64 || o + 40 + n > v.byteLength) throw new Error("Truncated payload");
            ...
            if (o !== v.byteLength) throw new Error("Trailing batch data");
            return result;
        }
        async function parseScl(buffer) {
            ...
            if (v.byteLength < 32 || v.getUint32(0, true) !== 0x314c4353 ||
                ...
                v.getUint32(8, true) !== 0x12345678 || v.getUint32(12, true) !== 0) throw new Error("Unsupported SCL header");
            let o = 32;
            while (o < v.byteLength && result.length < LIMIT) {
                if (o + 56 > v.byteLength) throw new Error("Truncated SCL record at byte " + o);
                const size = v.getUint16(o, true), n = v.getUint8(o + 8);
                if (v.getUint8(o + 2) !== 1 || n > 64 || size !== 56 + n ||
                    o + size > v.byteLength) throw new Error("Invalid SCL record at byte " + o);
                result.push(validate({
                ...
                o += size;
                if (result.length % 2000 === 0) await new Promise(resolve => setTimeout(resolve, 0));
            }
        ...
        function subscribe(next) {
            if (!socket || socket.readyState !== 1 || pending) return;
        ...
        function rebuild() {
            ...
            if (tokens.some(t => !/^(\?\?|[0-9a-fA-F]{2})$/.test(t))) {
                ...
                $("events").textContent = ""; $("ids").textContent = "";
                groups = []; filtered = [];
            ...
            if ($("order").value === "new") filtered.reverse();
        ...
        function inspect(index) {
            const e = records[index]; if (!e) return;
            let previous = null;
            for (let i = index - 1; i >= 0; i--) {
                if (key(records[i]) === key(e) && records[i].type === e.type) { previous = records[i]; break; }
            }
...
(() => {
    ...
    const nominalRates = [10000, 20000, 33333, 50000, 83333, 100000,
        125000, 250000, 500000, 800000, 1000000];
    const dataRates = [1000000, 2000000, 4000000, 5000000];
    const rateOptions = rates => rates.map(rate =>
        `<option value="${rate}">${rate >= 1000000
            ? `${rate / 1000000} Mbit/s` : `${rate / 1000} kbit/s`}</option>`).join("");
        ...
        return `<fieldset class="can-config-channel" data-bus="${bus}">
            ...
            <div class="can-config-fields">
                ...
                ${secondary ? `<label>Format<select data-field="format">
                    ...
                    <label>Data speed<select data-field="data">${rateOptions(dataRates)}</select></label>` : ""}
            </div>
            <div class="can-config-card-footer">
                <span title="No termination control is exposed by this firmware">120 Ω · Not supported</span>
                <button type="button" data-action="apply">Apply ${secondary ? "Secondary" : "Primary"}</button>
            </div>
        </fieldset>`;
    }
    ...
    function updateControls() {
        ...
        host.querySelector('[data-action="save"]').disabled = busy || !loaded || dirty.size > 0;
    }
    ...
    function populate(panel, config) {
        ...
        field(panel, "rate").value = String(secondary ? config.nominal_bitrate : config.bitrate);
        if (secondary) {
            if (config.brs_enabled && (!config.fd_enabled ||
                !dataRates.includes(config.data_bitrate) || config.data_bitrate <= config.nominal_bitrate)) {
                throw new Error("Unsupported CAN FD settings response");
            }
            field(panel, "format").value = config.fd_enabled ? (config.brs_enabled ? "brs" : "fd") : "classic";
            field(panel, "data").value = String(dataRates.includes(config.data_bitrate) ? config.data_bitrate : 2000000);
        }
    }

    async function reload() {
        if (busy || (dirty.size && !window.confirm("Discard unapplied CAN changes?"))) return;
        busy = true;
        updateControls();
        message("Loading CAN settings…");
        try {
            const settings = await request("/api/settings");
            for (const panel of panels) populate(panel, settings.can?.[panel.dataset.bus]);
            loaded = true;
            dirty.clear();
    ...
    async function apply(panel) {
        ...
        if (!window.confirm(`Apply ${bus} CAN settings? This may interrupt capture or recording on that bus.`)) return;
        ...
        if (bus === "primary") {
            ...
            config.data_bitrate = config.brs_enabled ? Number(field(panel, "data").value) : rate;
        }
        busy = true;
        updateControls();
        ...
        try {
            ...
            dirty.delete(bus);
            message(`${bus === "primary" ? "Primary" : "Secondary"} applied. Not saved to flash yet.`);
        } catch (error) {
            loaded = false;
            message(`Apply/readback failed: ${error.message}. Reload to check device state.`, true);
        } finally {
            busy = false;
            updateControls();
        }
    ...
    for (const panel of panels) {
        ...
        panel.querySelector('[data-action="apply"]').addEventListener("click", () => apply(panel));
    }
    ...
    host.querySelector('[data-action="save"]').addEventListener("click", async () => {
        if (busy || !loaded || dirty.size) return;
        if (!window.confirm("Save all currently applied device settings to flash?")) return;
        busy = true;
        updateControls();
        try {
    ...
    const pages = {'page-overview': init_overview, 'page-settings': init_settings, 'page-files': init_files, 'page-test': init_test, 'page-logger': init_logger, 'page-analyzer': init_analyzer};
    ...
    for (const [page, initialize] of Object.entries(pages)) {
        ...
        if (document.body.classList.contains(page)) {
            ...
            if (page === 'page-logger' || page === 'page-analyzer') init_can_settings();
